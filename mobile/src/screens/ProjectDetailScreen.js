...
import {
  ...
  Platform, Modal, Pressable, RefreshControl,
} from 'react-native';
...
export default function ProjectDetailScreen({ route, navigation }) {
  ...
  const loadPhotos = useCallback(async () => {
    try {
      const data = await getPhotos(project.id);
  ...
  const pickImage = async (fromCamera) => {
    ...
    if (!result.canceled && result.assets?.[0]) {
      navigation.navigate('AnnotatePhoto', { imageUri: result.assets[0].uri, projectId: project.id });
    }
  };
  ...
  const handleCreatePPT = async () => {
    if (groupedPhotos.length === 0) {
      Alert.alert('No Photos', 'Add at least one photo before generating a presentation.');
      return;
    }
    setGenerating(true);
    try {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => { await deleteProject(project.id); navigation.goBack(); },
        },
      ]
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={[colors.primaryDark, colors.primary]} style={styles.header}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={styles.headerSub} numberOfLines={1}>Client: {project.client_name}</Text>
          </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.stat}>
            ...
            <Text style={styles.statText}>{groupedPhotos.length} Photos</Text>
          </View>
          {!!project.description && (
      ...
      {loading ? (
        ...
        <FlatList
          data={groupedPhotos}
          keyExtractor={(group) => group[0].image_url}
          renderItem={({ item: group }) => (
            <PhotoCard
              group={group}
              onPress={() => navigation.navigate('PhotoDetail', { photos: group, projectId: project.id })}
            />
          )}
          numColumns={2}
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>Tap the camera button to add your first banner photo</Text>
            </View>
          }
          columnWrapperStyle={{ justifyContent: 'flex-start' }}
          refreshControl={<RefreshControl refreshing={false} onRefresh={loadPhotos} colors={[colors.primary]} />}
        />
      )}
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity
          ...
          onPress={handleCreatePPT} disabled={generating} activeOpacity={0.85}
        >
          {generating
            ? <><ActivityIndicator size="small" color="#fff" /><Text style={styles.pptBtnText}>Generating...</Text></>
            : <><Ionicons name="document-text" size={20} color="#fff" /><Text style={styles.pptBtnText}>Create PPT</Text></>
          }
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  statsRow: {
    ...
    backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 10, padding: 10, gap: 10,
  },
