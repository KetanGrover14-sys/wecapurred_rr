...
const SCHEMA = {
  ...
  photos:        ['id', 'project_id', 'image_url', 's3_key', 'location', 'store_name', 'store_owner_name', 'store_owner_mobile', 'length', 'breadth', 'height', 'material', 'collateral', 'notes', 'created_at'],
  users:         ['id', 'email', 'password_hash', 'name', 'role'],
  project_files: ['id', 'project_id', 'type', 'file_name', 'file_url', 's3_key', 'uploaded_by', 'uploaded_by_name', 'status', 'reviewed_by', 'reviewed_by_name', 'reviewed_at', 'created_at'],
};
...
export async function getRepositoryData(user) {
  ...
  return { projects, photos: visible(photos), files: visible(files).filter(f => f.type === 'installation'), mappings: visible(mappings) };
}
