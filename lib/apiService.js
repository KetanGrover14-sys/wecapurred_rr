...
export const addPhoto = async (projectId, location, storeName, storeOwnerName, storeOwnerMobile, entries, file, onProgress, details = {}) => {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('location', location);
  formData.append('store_name', storeName);
  formData.append('store_id', details.store_id || '');
  formData.append('brand_name', details.brand_name || '');
...
export const uploadProjectFile = (projectId, type, file, onProgress, photoId, removalDate, expiryDays) => {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('type', type);
  if (photoId) formData.append('photo_id', photoId);
  if (removalDate) formData.append('removal_date', removalDate);
  if (expiryDays) formData.append('project_expiry_days', expiryDays);
  return new Promise((resolve, reject) => {
